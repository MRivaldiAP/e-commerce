...
class CheckoutController extends Controller
{
    ...
    public function createPaymentSession(Request $request, PaymentGatewayManager $payments, ShippingGatewayManager $shipping): JsonResponse
    {
        ...
        $cartSummary = Cart::summary();
        $shippingCost = $shippingEnabled ? (float) Arr::get($shippingData, 'cost', 0) : 0.0;
        $cart = [
        ...
        try {
            ...
            if ($shippingEnabled) {
                ...
                $request->merge([
                    'customer' => [
                        'email' => Arr::get($contact, 'email', $request->input('customer.email', 'guest@local.test')),
                        'first_name' => Arr::get($contact, 'name', $request->input('customer.first_name', 'Guest Customer')),
            ...
            $address = $this->resolveCheckoutAddress($user, Arr::get($shippingData, 'address', []));
    ...
    protected function resolveCheckoutAddress(User $user, array $shippingAddress = []): Address
    {
        ...
        $payload = [
            'recipient_name' => Arr::get($shippingAddress, 'recipient', $user->name ?? 'Pelanggan'),
            'phone' => Arr::get($shippingAddress, 'phone', $user->phone ?? '0000000000'),
            'street' => Arr::get($shippingAddress, 'street', 'Alamat belum diatur'),
            'village' => Arr::get($shippingAddress, 'village_name', 'Belum diatur'),
            'subdistrict' => Arr::get($shippingAddress, 'district_name', 'Belum diatur'),
            'city' => Arr::get($shippingAddress, 'regency_name', 'Belum diatur'),
            'province' => Arr::get($shippingAddress, 'province_name', 'Belum diatur'),
            'postal_code' => Arr::get($shippingAddress, 'postal_code', '00000'),
            'is_default' => true,
        ];
