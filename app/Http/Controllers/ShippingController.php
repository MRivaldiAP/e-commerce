...
class ShippingController extends Controller
{
    public function index(Request $request, ShippingGatewayManager $shipping): View|RedirectResponse
    {
        ...
        $viewCandidates = [];
        if ($gateway) {
            $viewCandidates[] = base_path("themes/{$theme}/views/shipping-{$gateway->key()}.blade.php");
        }
        $viewCandidates[] = base_path("themes/{$theme}/views/shipping.blade.php");

        $viewPath = collect($viewCandidates)->first(function ($path) {
